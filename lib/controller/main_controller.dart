...
class MainController {
  ...
  Future<UserCredential> signInWithGoogle() async {
    try {
      ...
      debugPrint(
          "-------authService => signInWithGoogle  =>  after get google auth --------");

      final GoogleSignInAuthentication? googleAuth =
      await googleUser?.authentication;

      debugPrint(
          "-------authService => signInWithGoogle  =>  before get google auth --------");

      debugPrint(
          "-------authService => signInWithGoogle  =>  after get credential --------");
      ...
      debugPrint(
          "-------authService => signInWithGoogle  =>  before get credential --------");

      debugPrint(
          "-------authService => signInWithGoogle  =>  after get userCredential --------");
      UserCredential userCredential =
      await _auth.signInWithCredential(credential);
      debugPrint(
          "-------authService => signInWithGoogle  =>  before get userCredential --------");
  ...
  static checkPremiumOrNot(UserModel userData) async {
    ...
    await _fireStore.collection('botUsers').doc(user.uid).get();
  ...
  static sendSupportTicket({
    ...
    try {
      ...
      debugPrint(
          "🐞🐞🐞 Printing Error PlatformException => ${e.message!}  🐞🐞🐞");
    } catch (e) {
      debugPrint(
          "🐞🐞🐞 Printing Error PlatformException => ${e.toString()}  🐞🐞🐞");
    }
  }
  ...
  static Future<void> addQuestionsToFirestore() async {
    try {
      ...
      for (var element in catList) {
        await reff
            .doc(element.catName)
            .set({"catName": element.catName, "questions": element.questions});
      }
  ...
  static getCredentials() async {
    ...
    final DocumentSnapshot userDoc =
    await _fireStore.collection('credentials').doc('manage-api-key').get();
  ...
  static updateToFreeUser() async {
    ...
    await _fireStore.collection('botUsers').doc(LocalStorage.getId()!).update({
      "isPremium": false,
      "textCount": 0,
      "imageCount": 0,
      "contentCount": 0,
...
class CateogrySuggetion {
  ...
  CateogrySuggetion({required this.catName, required this.questions});
}
...
List<CateogrySuggetion> catList = [
  CateogrySuggetion(catName: "Interview", questions: interviewQuestions),
  CateogrySuggetion(
      catName: "Business Assistant", questions: businessAssistantQuestions),
  CateogrySuggetion(catName: "Developer", questions: developerQuestions),
  CateogrySuggetion(catName: "Advertiser", questions: advertiserQuestions),
  CateogrySuggetion(
      catName: "Language Expert", questions: languageExpertQuestions),
  CateogrySuggetion(
      catName: "E-mail Writing", questions: emailWritingQuestions),
  CateogrySuggetion(catName: "Chef", questions: chefQuestions),
  CateogrySuggetion(catName: "Mental Health", questions: mentalHealthQuestions),
  CateogrySuggetion(
      catName: "Self Improvement", questions: selfImprovementQuestions),
];
